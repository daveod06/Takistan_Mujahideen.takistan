...
_HC1_ID = -1; // Will become the Client ID of HC1
...
while {true} do {
  ...
  try {
    ...
    if (_HC1_ID > 2) then {
      diag_log format ["passToHCs: Found HC1 with Client ID %1", _HC1_ID];
    } else { 
      diag_log "passToHCs: [WARN] HC1 disconnected";

      HC1 = objNull;
      _HC1_ID = -1;
    };
  } catch { diag_log format ["passToHCs: [ERROR] [HC1] %1", _exception]; HC1 = objNull; _HC1_ID = -1; };
  ...
  if (!isNil "HC3") then {
    try {
      ...
      if (_HC3_ID > 2) then {
        diag_log format ["passToHCs: Found HC3 with Client ID %1", _HC3_ID];
      } else { 
  ...
  if ( (isNull HC1) && (isNull HC2) && (isNull HC3) ) then { waitUntil {!isNull HC1}; };  

  // Check to auto enable Round-Robin load balancing strategy
  if ( (!isNull HC1 && !isNull HC2) || (!isNull HC1 && !isNull HC3) || (!isNull HC2 && !isNull HC3) ) then { _loadBalance = true; };
